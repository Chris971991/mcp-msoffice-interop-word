...
import { registerVbaModuleTools } from "./tools/vba-module-tools.js";
import { registerActiveXControlTools } from "./tools/activex-control-tools.js";
import { registerVbaExecutionTools } from "./tools/vba-execution-tools.js";
import { registerVbaTemplateTools } from "./tools/vba-template-tools.js";
...
registerVbaModuleTools(mcpServer);
registerActiveXControlTools(mcpServer);
registerVbaExecutionTools(mcpServer);
registerVbaTemplateTools(mcpServer);
...
async function startServer() {
  ...
  if (transportMode === 'stdio') {
    debug.log("Starting MCP server with stdio transport...");
    const transport = new StdioServerTransport();
    await mcpServer.connect(transport);
    debug.log("MCP server connected via stdio.");
    ...
    app.get("/sse", async (_req: Request, res: Response) => {
      debug.log("SSE connection requested");
      const transport = new SSEServerTransport('/messages', res);
      transports[transport.sessionId] = transport;
      debug.log(`SSE transport created with session ID: ${transport.sessionId}`);

      res.on("close", () => {
        debug.log(`SSE connection closed for session ID: ${transport.sessionId}`);
      ...
      try {
        await mcpServer.connect(transport);
        debug.log(`MCP server connected via SSE for session ID: ${transport.sessionId}`);
      } catch (error) {
        debug.error(`Error connecting MCP server via SSE for session ID: ${transport.sessionId}`, error);
    ...
    app.post("/messages", async (req: Request, res: Response) => {
      ...
      debug.log(`Received POST message for session ID: ${sessionId}`);
      const transport = transports[sessionId];
      if (transport) {
        try {
          // Pass the already parsed body if using express.json()
          await transport.handlePostMessage(req, res, req.body);
          debug.log(`Handled POST message for session ID: ${sessionId}`);
        } catch (error) {
          debug.error(`Error handling POST message for session ID: ${sessionId}`, error);
          if (!res.headersSent) {
             res.status(400).send('Error processing message');
          }
        }
      } else {
        debug.warn(`No active SSE transport found for session ID: ${sessionId}`);
        res.status(400).send('No transport found for sessionId');
      }
    });

    httpServer.listen(port, () => {
      debug.log(`MCP server listening with SSE transport on port ${port}`);
      debug.log(`SSE endpoint: /sse`);
      debug.log(`Message endpoint: /messages`);
    });

  } else {
    debug.critical(`Unsupported MCP_TRANSPORT: ${transportMode}. Use 'stdio' or 'sse'.`);
    process.exit(1);
  }
}

startServer().catch(error => {
  debug.critical("Failed to start MCP server:", error);
  process.exit(1);
});
